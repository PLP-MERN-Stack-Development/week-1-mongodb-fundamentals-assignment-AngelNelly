...
async function run() {
  try {
    ...
    // 2. Find books by author "Frank Herbert"
    const booksByAuthor = await books.find({ author: "Frank Herbert" }).toArray();
    console.log("\n2. Books by Frank Herbert:");
    ...
    // 5. Update price of "Dune"
    const updateResult = await books.updateOne(
      { title: "Dune" },
      { $set: { price: 18.99 } }
    );
    console.log(`\n5. Updated Dune price: Matched ${updateResult.matchedCount}, Modified ${updateResult.modifiedCount}`);

    // 6. Delete book titled "Beloved"
    const deleteResult = await books.deleteOne({ title: "Beloved" });
